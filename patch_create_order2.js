...
const regex = /async function createOrder\(userId, amount, upi, userObj = null, overrideAmount = null, precisePayableAmount = null\) \{[\s\S]*?return null;\n\s*\}\n\}/;

const start = s.indexOf('async function createOrder(userId, amount, upi, userObj = null, overrideAmount = null, precisePayableAmount = null) {');
if (start !== -1) {
    let braceCount = 0;
    let end = -1;
    ...
    if (end !== -1) {
        const replacement = `async function createOrder(userId, amount, upi, userObj = null, overrideAmount = null, precisePayableAmount = null) {
    ...
    try {
        ...
        if (!user) { releaseLock(cleanPhone); return null; }
        ...
        if (totalActiveAmount + selectedAmount > walletBalance) {
            console.log(\`[createOrder] Blocked: Order amount exceeds balance\`);
            releaseLock(cleanPhone);
            return null;
        }
        ...
        // MODULE 2: Dynamic Paisa Discount Engine
        ...
        if (payableAmount == null) {
            for (let i = 1; i <= 99; i++) {
                ...
                const isUsed = sameBaseOrders.some(o => Math.abs((o.payable_amount || o.amount) - potentialPayable) < 0.001);
                if (!isUsed) {
        ...
        console.log("Replaced createOrder correctly.");
    }
}
